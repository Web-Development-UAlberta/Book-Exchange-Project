...
public class ListingServiceIntegrationTests : IDisposable
{
    ...
    public ListingServiceIntegrationTests()
    {
        ...
        _matchingServiceMock
            .Setup(m => m.CreateMatchNotificationsAsync(It.IsAny<Listing>()))
            .Returns(Task.CompletedTask);

        _service = new ListingService(_db, _matchingServiceMock.Object);
    }
    ...
    public async Task IT_LIST_01_AuthenticatedUserCreatesListing_ListingIsSavedAndReturned()
    {
        ...
        _matchingServiceMock.Verify(
            m => m.CreateMatchNotificationsAsync(It.Is<Listing>(l => l.Id == result.Id)),
            Times.Once);
    }
    ...
    public async Task IT_LIST_02_CreateListingWithInvalidIsbn_IsBlockedAndDatabaseUnchanged()
    {
        ...
        _matchingServiceMock.Verify(
            m => m.CreateMatchNotificationsAsync(It.IsAny<Listing>()),
            Times.Never);
    }
