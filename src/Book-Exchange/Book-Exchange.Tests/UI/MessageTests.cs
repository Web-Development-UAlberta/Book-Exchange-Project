...
// ///   Optionally, one message should be linked to an ExchangeRequest (UI-MSG-21).
...
//     /// <summary>
//     /// UI-MSG-17: Offer bubble elements are visible when a message has an
//     /// ExchangeRequest context.
//     /// Expected: offer message, offer accept btn, offer decline btn are rendered.
//     /// Skips gracefully if no exchange-linked message exists in seed data.
//     /// </summary>
//     [Fact]
//     public async Task UI_MSG_17_Conversation_OfferBubble_IsVisibleForExchangeMessages()
//     {
//         await LoginAsync();
//         var url = await GetFirstConversationUrlAsync();
//         await Page.GotoAsync(url);
...
//         var offerCount = await Page.Locator(".offer-message").CountAsync();
//         if (offerCount == 0) return;   // no exchange-linked seed data – skip

//         await Expect(Page.Locator(".offer-message").First).ToBeVisibleAsync();
//         await Expect(Page.Locator(".offer-accept-btn").First).ToBeVisibleAsync();
//         await Expect(Page.Locator(".offer-decline-btn").First).ToBeVisibleAsync();
//     }
// }