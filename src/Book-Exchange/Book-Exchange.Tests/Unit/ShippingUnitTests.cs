...
public class ShippingServiceUnitTests
{
    ...
    // Real service instance used for pure method tests (CalculateShippingCost)
    // that have no dependencies and should not be mocked.
    ...
    public ShippingServiceUnitTests()
    {
        ...
        // Real instance with fake dependencies so we can call
        // the method directly without hitting external services.
        ...
        _realService = new ShippingService(
            db,
            httpClient,
            config,
    ...
    public void UT_SHIP_01_CalculateShippingCost_ValidInputs_ReturnsCorrectCost()
    {
        ...
        _serviceMock
            .Setup(s => s.CalculateShippingCost(
                carrier,
                weightGrams,
                distanceKm))
            .Returns(expectedCost);

        var result = _serviceMock.Object.CalculateShippingCost(
            carrier,
            weightGrams,
            distanceKm);
    ...
    /// </summary>
    /// <returns>
    /// InvalidOperationException with message indicating
    /// the package weight exceeds the carrier's maximum
    /// </returns>
    [Fact]
    public async Task UT_SHIP_02_WeightExceedsCarrierMax_ThrowsInvalidOperationException()
    {
        ...
        _serviceMock
            ...
            .ThrowsAsync(new InvalidOperationException(
                "Package weight exceeds the carrier's maximum allowed weight."));

        await Assert.ThrowsAsync<InvalidOperationException>(
            () => _serviceMock.Object.CreateShipmentAsync(
                transactionId,
                senderAddressId,
                receiverAddressId,
                carrierId,
    ...
    /// </summary>
    /// <returns>
    /// Positive decimal representing the distance in kilometres
    ...
    public async Task UT_SHIP_04_GetDistanceKm_ValidPlaceIds_ReturnsDistance()
    {
        ...
        _serviceMock
            .Setup(s => s.GetDistanceKmAsync(
                senderPlaceId,
                receiverPlaceId))
            .ReturnsAsync(expectedDistance);

        var result = await _serviceMock.Object.GetDistanceKmAsync(
            senderPlaceId,
            receiverPlaceId);
    ...
    /// </summary>
    /// <returns>
    /// InvalidOperationException indicating
    /// the distance lookup failed
    /// </returns>
    [Fact]
    public async Task UT_SHIP_05_GetDistanceKm_ApiFails_ThrowsInvalidOperationException()
    {
        ...
        _serviceMock
            .Setup(s => s.GetDistanceKmAsync(
                ...
                receiverPlaceId))
            .ThrowsAsync(new InvalidOperationException(
                "Distance lookup failed. Unable to reach the distance API."));

        await Assert.ThrowsAsync<InvalidOperationException>(
            () => _serviceMock.Object.GetDistanceKmAsync(
                senderPlaceId,
                receiverPlaceId));
    }
    ...
    /// <summary>
    /// UT-SHIP-06:
    /// Weight that does not divide evenly by 1000
    /// produces correct decimal cost
    ///
    /// 1500g = 1.5kg
    ///
    /// Expected:
    /// 5.00 + (1.5 × 3.50) + (100 × 0.02)
    /// = 5.00 + 5.25 + 2.00
    /// = 12.25
    /// </summary>
    /// <returns>
    /// Decimal cost calculated using the formula
    /// with correct decimal arithmetic
    /// </returns>
    [Fact]
    public void UT_SHIP_06_CalculateShippingCost_NonEvenWeightKg_NoDecimalDrift()
    {
        ...
        var result = _realService.CalculateShippingCost(
            carrier,
            1500,
            100m);
    ...
    /// <summary>
    /// UT-SHIP-07:
    /// Very small weight (1g)
    /// does not produce a negative or zero cost
    ///
    /// 1g = 0.001kg
    ///
    /// Expected:
    /// Result is positive and greater than BaseCost
    /// </summary>
    /// <returns>
    /// Decimal cost calculated using the formula
    /// with correct decimal arithmetic
    ...
    /// <summary>
    /// UT-SHIP-08:
    /// Weight that produces a repeating decimal
    /// in intermediate math is handled correctly
    ///
    /// 333g = 0.333kg
    ///
    /// Expected:
    /// 5.00 + (0.333 × 3.00) + (50 × 0.01)
    /// = 5.00 + 0.999 + 0.50
    /// = 6.499
    /// </summary>
    /// <returns>
    ...
    /// <summary>
    /// UT-SHIP-09:
    /// Zero weight throws ArgumentException
    ///
    /// Expected:
    /// ArgumentException is thrown before calculation
    /// </summary>
    /// <returns>
    ...
    public void UT_SHIP_09_CalculateShippingCost_ZeroWeight_ThrowsArgumentException()
    {
        ...
        Assert.Throws<ArgumentException>(
            () => _realService.CalculateShippingCost(
                carrier,
                0,
                100m));
    }
...
// Helper: no-op HTTP handler
// ShippingService requires HttpClient in constructor
